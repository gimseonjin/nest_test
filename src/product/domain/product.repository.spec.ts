...
describe("Product Respository", () => {
  ...
  it("원하는 판매 상태를 가진 상품들을 조회한다.", () => {
    ...
    // when
    const result = repo.findAllBySellingTypeIn([ProductSellingType.SELLING, ProductSellingType.STOP_SELLING])

    // then
    expect(result.length).toBe(2)
    expect(result).toMatchObject([
      {productNumber: "001", name: "아메리카노", productSellingType: ProductSellingType.SELLING},
      {productNumber: "003", name: "크루아상", productSellingType: ProductSellingType.STOP_SELLING}
    ])
  })
